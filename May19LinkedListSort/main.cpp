...
int main() {
  ...
  size++;
  printNode();
  Node *temp1 = NULL;    //1st time will be NULL
  bool isSwapped;
  do {
    temp = head;
    isSwapped = false; //bubble sort
    while (temp->next != temp1) { //temp1 1st time then reduce to last - 1
    ...
    temp1 = temp;             //after 1st time store temo1 will be next to last
  } while (isSwapped);
  printNode();
}
